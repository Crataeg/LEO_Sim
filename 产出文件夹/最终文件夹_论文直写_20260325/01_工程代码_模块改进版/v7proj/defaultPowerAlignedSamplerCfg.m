function cfg = defaultPowerAlignedSamplerCfg()
    ...
    % Hierarchical label rule:
    % 1) detect whether interference is visible enough
    % 2) if visible, assign a morphology label
    cfg.LabelRule = struct( ...
        'MinInterfOverNoise_dB', 3.0, ...
        'MinJamOverSignal_dB', -10.0, ...
        'MinCCIOverSignal_dB', -8.0);

    % Training-domain parameters for the dataset generator.
    % The new generator intentionally covers weaker jammer cases than the
    % original [0,12] dB JSR range to reduce the train/test domain gap.
    cfg.Train = struct();
    cfg.Train.SNRdB_list = [-8 -5 -2 0 3 6];
    cfg.Train.NoneJSR_dB_Range = [-30 -12];
    cfg.Train.NoneISR_dB_Range = [-30 -10];
    cfg.Train.JamJSR_dB_Range  = [-8 12];
    cfg.Train.CCIISR_dB_Range  = [-18 -3];
end
