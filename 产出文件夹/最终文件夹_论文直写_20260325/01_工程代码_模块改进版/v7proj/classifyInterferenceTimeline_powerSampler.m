function [trueLab, predLab, scoreMat, snapInfo] = classifyInterferenceTimeline_powerSampler( ...
    ...
    for k=1:numSteps
        ...
        [clsTrue, labelMeta] = inferTrueClassFromPowers(Ps, Pi, Pj, Pn, cfg); %#ok<NASGU>
        ...
        % if the power rule says interference is not visible, force "none".
        if strcmp(char(clsTrue), 'none')
            predLab(k) = categorical("none", classes);
            scoreMat(k,:) = 0;
            scoreMat(k, find(strcmp(classes, 'none'),1)) = 1;
        else
            try
